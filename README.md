...
단기 AI 추천 외에, DART 공시 기반 펀더멘털(PER·PBR·ROE·부채비율)과 **Piotroski F-Score**를 활용한 **가치주 스크리닝**도 지원합니다. 중기(3~6개월) 관점의 저평가 우량주를 대시보드에서 직접 필터링하고 가치점수(value_score) 순으로 조회할 수 있습니다.
...
```mermaid
graph TD
    CLI["🖥 CLI<br/>serve · recommend · analyze · train<br/>outcomes · sync · init · home · value"]
    USER["👤 사용자<br/>브라우저"]

    subgraph API["⚡ FastAPI  (koreanstocks.api)"]
        RA["recommendations<br/>analysis · watchlist<br/>backtest · market · models"]
        RV["value<br/>GET /api/value_stocks"]
    end

    subgraph FRONTEND["🌐 Frontend (dashboard.html — 7탭)"]
    ...
    subgraph PIPELINE_VAL["💰 중기 가치주 스크리닝 파이프라인"]
    ...
    CLI -->|koreanstocks value| PIPELINE_VAL
