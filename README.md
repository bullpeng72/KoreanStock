...
│   │   └── database.py              # SQLite 관리 (분석 결과 및 워치리스트)
│   ├── engine/
...
KRX 전체 상장 종목
    ├── 당일 거래량 상위 50종목
    └── 당일 등락률 상위 50종목
            ↓ 합집합 (KOSPI + KOSDAQ만 필터)
        최대 ~100개 후보
            ↓ 거래량 순 정렬 후 상위 20개 심층 분석
        최종 5종목 추천
```
...
1단계  기술적 지표 계산            → tech_score (0~100)
       SMA 5/20/60/120, MACD, RSI(14), Bollinger Bands, OBV

2단계  ML 앙상블 예측              → ml_score (0~100)
       Random Forest + Gradient Boosting + XGBoost
       피처: RSI, MACD diff, 가격/SMA20 비율, 거래량 변화율
       모델 없을 경우 tech_score 폴백

3단계  뉴스 감성 분석              → sentiment_score (-100~100)
       Naver News API → 최신 뉴스 10개 → GPT-4o-mini 감성 점수
...
최종 종합 점수 = tech × 0.3 + ml × 0.4 + normalized_sentiment × 0.3
```
...
| 점수 | 해석 | 구성 |
|------|------|------|
| 80~100 | 매우 강세 | 추세(40) + 모멘텀(30) + 변동성(30) 대부분 충족 |
| 60~79 | 강세 | 주요 신호 긍정적 |
| 40~59 | 중립 | 혼재 신호 |
| 0~39 | 약세 | 대부분 부정적 |

**세부 구성:**
- 추세 (40점): 종가 > SMA20 (+10), SMA5 > SMA20 (+10), MACD 골든크로스 (+20)
- 모멘텀 (30점): RSI 40~60 안정구간 (+30), RSI 30~40 진입구간 (+20), RSI 60~70 (+15)
- 변동성 (30점): 볼린저 밴드 중간 범위 내 (+30)
...
**실행 시점:** 평일 오전 16:30 KST (UTC 07:30) — 장 마감 후 자동 실행
...
TELEGRAM_BOT_TOKEN
TELEGRAM_CHAT_ID
NAVER_CLIENT_ID      (선택 — 없으면 뉴스 감성 분석 스킵)
...
| **AI Recommendations** | 테마·시장별 추천 생성, 날짜 선택으로 히스토리 조회, 세션 캐시 |
| **Backtest Viewer** | RSI/MACD/COMPOSITE 전략 수익률·MDD·샤프비율 시뮬레이션 |
