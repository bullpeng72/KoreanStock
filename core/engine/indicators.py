...
class IndicatorCalculator:
    ...
    def get_composite_score(self, df: pd.DataFrame) -> float:
        ...
        # 2. 모멘텀 점수 (30%)
        # RSI 구간별 점수: 안정 구간(40~60)이 최고점, 극단 구간일수록 낮아짐
        mom_score = 0
        rsi = latest['rsi']
        if 40 <= rsi <= 60:   mom_score += 30  # 중립 안정 구간 (최적)
        elif 30 < rsi < 40:   mom_score += 20  # 매수 진입 구간
        elif 60 < rsi <= 70:  mom_score += 15  # 모멘텀 상승 구간
        elif rsi <= 30:       mom_score += 10  # 과매도 (반등 기대, 리스크 있음)
        else:                 mom_score += 5   # RSI > 70: 과매수 (오버히팅)
        
        # 3. 변동성 및 가격 위치 (30%)
        vol_score = 0
        bb_pos = (latest['close'] - latest['bb_low']) / (latest['bb_high'] - latest['bb_low']) if (latest['bb_high'] - latest['bb_low']) != 0 else 0.5
        if 0.1 < bb_pos < 0.9: vol_score += 30
