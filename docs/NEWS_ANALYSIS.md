...
| 검색어 | `"{종목명} 주가"` |
| 수집 수 | `display=30` (중복·계열사 제거 후 여유분 확보) |
| 정렬 | `sort=date` (최신순) |
...
| 우선순위 | 항목 | 기대 효과 |
|---------|------|---------|
| 1 | corpCode.xml SQLite 영속화 — 당일 1회 다운로드 제한 | API 비용 절감 |
| 2 | DART 주요사항(B) 공시 금액·비율 파싱 | 분석 정확도 향상 |
| 3 | 감성 점수 calibration — 분포 정규화로 전범위 활용 | 가중치 실효성 향상 |
